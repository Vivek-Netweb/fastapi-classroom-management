...
async def get_profile(request: Request,  conn : asyncpg.Connection = Depends(get_conn_dependency)):
    user = request.state.user
    print("user auth")
...
@user_routes.get("/")
async def get_all_users(request : Request, _:None = Depends(required_roles(["admin"])),  conn : asyncpg.Connection = Depends(get_conn_dependency)):
